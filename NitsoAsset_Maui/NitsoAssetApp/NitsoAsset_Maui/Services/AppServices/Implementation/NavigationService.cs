...
using NitsoAsset_Maui.Services.AppServices.PageLocator;
...
namespace NitsoAsset_Maui.Services.AppServices.Implementation
{
    public class NavigationService : INavigationService
    {
        ...
        public void SetMainViewModel<T>(object args = null) where T : IViewModel
        {
            Action setmainView = () =>
            {
                try
                {
                    ...
                    var masterDetailPage = page as FlyoutPage;
            ...
            {
                MainThread.BeginInvokeOnMainThread(setmainView);
            }
            else
            {
        ...
        {
            var masterDetailPage = App.Current.MainPage as FlyoutPage;
        ...
        public Task ShowPopup<T>(object args = null) where T : IViewModel
        {
            try
            {
                ...
                if (page == null)
                {
                    var msg = "No page found for args: " + (args == null ? "null" : args?.GetType() + " " + args);
                    return Task.FromResult(false);
                }
                ...
                if (popup == null)
                {
                    Debug.WriteLine("Resolved page is not a PopupPage.");
                    return Task.FromResult(false);
                }

                if ((DateTime.Now - _lastNavTime).Seconds > 1)
                {
                    var tcs = new TaskCompletionSource<bool>();
